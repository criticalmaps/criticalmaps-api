    ...
    .write(pathThumbnail, (err) => {
      ...
      fs.readFile(pathImage, (err, dataImage) => {
        fs.readFile(pathThumbnail, (err, dataThumbnail) => {
          postgres_db
            .none(
              "INSERT INTO gallery(image, thumbnail, review_state, ip, longitude, latitude) \
            VALUES($1, $2, $3, $4, $5, $6)",
              [
                dataImage,
                dataThumbnail,
...
router.get("/", (_req, res) => {
  ...
  postgres_db
    .any("SELECT id, longitude, latitude FROM gallery")
    .then((data) => {
      res.send(data);
    });
});
...
router.get("/thumbnail/:id", (req, res) => {
  res.setHeader("Access-Control-Allow-Origin", "*");
  postgres_db
    .one("SELECT thumbnail FROM gallery WHERE id=$1", [req.params.id])
    .then((data) => {
      console.log(data);
      res.writeHead(200, {
...
router.get("/image/:id", (req, res) => {
  res.setHeader("Access-Control-Allow-Origin", "*");
  postgres_db
    .one("SELECT image FROM gallery WHERE id=$1", [req.params.id])
    .then((data) => {
      console.log(data);
      res.writeHead(200, {
        "Content-Type": "image/jpeg",
